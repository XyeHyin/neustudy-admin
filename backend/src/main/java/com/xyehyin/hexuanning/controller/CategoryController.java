...
public class CategoryController extends BaseController {
    ...
    public ApiResponse<CategoryFlatVO> create(@RequestBody @Valid CreateCategoryDTO createCategoryDTO) {
        ...
        if (parentId != null) {
            ...
            if (categoryService.existsByNameAndParentId(createCategoryDTO.getName(), parentId)) {
    ...
    public ApiResponse<CategoryFlatVO> update(@PathVariable Long categoryId, @RequestBody @Valid UpdateCategoryDTO updateCategoryDTO) {
        ...
        if (parentId != null) {
            ...
            if (categoryService.existsByNameAndParentIdExcludingId(updateCategoryDTO.getName(), parentId, categoryId)) {
                throw new StatefulException(HttpStatus.HTTP_BAD_REQUEST, "父级分类下已存在同名分类");
            }
        } else if (categoryService.existsByNameAndParentIdExcludingId(updateCategoryDTO.getName(), null, categoryId)) {
            throw new StatefulException(HttpStatus.HTTP_CONFLICT, "分类名称已存在");
        }
        category.setParent(parent);
    ...
    public ApiResponse<List<CategoryTreeVO>> treeList() {
        ...
        List<CategoryTreeVO> tree = categories.stream()
                .filter(category -> category.getParent() == null)
                .map(category -> toCategoryTreeVO(category, childrenByParentId))
                .toList();
    ...
    private CategoryTreeVO toCategoryTreeVO(Category category, Map<Long, List<Category>> childrenByParentId) {
        ...
        vo.setChildren(childrenByParentId.getOrDefault(category.getId(), List.of()).stream()
                .map(child -> toCategoryTreeVO(child, childrenByParentId))
                .toList());
        return vo;
    }
}
