<template>
    <div class="knowledge-points-page">
      <n-card :bordered="false" class="knowledge-points-card">
        ...
        <n-data-table
          v-permission="'knowledge_point:list:self'"
          remote
          :columns="columns"
...
import { getEnabledStatusText as getStatusText, getEnabledStatusType as getStatusType } from '@/utils/status'
  ...
  const { loading: loadingKnowledgePoints, run: fetchKnowledgePoints } = useRequest(
    () => {
      const params = {
        ...
        return getMyKnowledgePointPage(params)
    },
    {
      onSuccess: (res: any) => {
        if (res.code === 200) {
  ...
  const { run: fetchCourses } = useRequest(getMyCourses, {
    onSuccess: (res: any) => {
      if (res.code === 200) {
  ...
  const { loading: creatingKnowledgePoint, runAsync: runCreateKnowledgePoint } = useRequest(createKnowledgePoint, { manual: true })
  const { loading: updatingKnowledgePoint, runAsync: runUpdateKnowledgePoint } = useRequest(updateKnowledgePoint, { manual: true })
